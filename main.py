...
from models import Host, Link, session
...
def import_hosts_menu():
    ...
    options = [
        "[y] Import from Ansible YAML",
        "[i] Import from Ansible INI",
        "[m] Manually add Host",
        "[b] Back to Main Menu",
    ]
    terminal_menu = TerminalMenu(
    ...
    # If "Import from Ansible YAML" is selected,
    #   the import_inv_from_yaml function is called
    if menu_entry_index == 0:
        import_inv_from_yaml()
    # If "Import from Ansible INI" is selected,
    #   the import_inv_from_ini function is called
    elif menu_entry_index == 1:
...
def import_links_menu():
    ...
    options = [
        "[c] Import from Containerlab Topology YAML",
        "[b] Back to Main Menu",
    ]
    terminal_menu = TerminalMenu(
        options,
...
def import_links_from_containerlab():
    ...
    if default_yaml_file:
        ...
        use_default = (
            input("Do you want to use the default topology file? (y/n): ")
            .strip()
            .lower()
        )
        if use_default == "y":
            ...
            yaml_file = input(
                "What is the path of your Containerlab topology file? "
            )
    else:
        yaml_file = input(
            "What is the path of your Containerlab topology file? "
...
def import_inv_from_yaml():
    ...
    if default_yaml_file:
        ...
        use_default = (
            input("Do you want to use the default inventory file? (y/n): ")
            .strip()
            .lower()
        )
        if use_default == "y":
        ...
        with open(yaml_file, "r", encoding="utf-8") as file:
            ...
            for group, group_data in children.items():
                ...
                for hostname, host_data in hosts.items():
                    ...
                    network_os = (
                        "junos"
                        if "junos" in image_type
                        else (
                            "linux"
                            if "linux" in image_type
                            else vars.get("ansible_network_os", "")
                        )
                    )
                    new_host = Host(
                        hostname=hostname,
        ...
        for section in config.sections():
            for hostname, ip_address in config.items(section):
                network_os = (
                    "junos"
                    if "junos" in section
                    else "linux" if "linux" in section else section
                )
                new_host = Host(
                    hostname=hostname,
                    ip_address=ip_address,
                    ...
                    connection=config.get(
                        section, "ansible_connection", fallback=""
                    ),
...
def connect_host_menu():
    ...
    options = [
        f"[{i}] {host.hostname}" for i, host in enumerate(hosts, start=1)
    ] + ["[b] Back to Main Menu"]
    terminal_menu = TerminalMenu(
        options,
        menu_cursor_style=("fg_red", "bold"),
    ...
    menu_entry_index = terminal_menu.show()
    # Logic to initiate connect to host or go back to main menu
    #   with a dynamic amount of options
...
def clear_screen():
    """
    Function to clear the screen. Used this when other TUI was used.
    Not needed for simple_term_menu.
    """
...
def find_ansible_inventory_yml():
    """
    Function to find ansible-inventory.yml in the current directory or
    clab folder in the parent directory.
    """
    ...
    for item in os.listdir(parent_dir):
        if item.startswith("clab") and os.path.isdir(
            os.path.join(parent_dir, item)
        ):
            clab_dir = os.path.join(parent_dir, item)
            ansible_inventory_yml = os.path.join(
                clab_dir, "ansible-inventory.yml"
            )
            if os.path.isfile(ansible_inventory_yml):
                return ansible_inventory_yml
...
def find_ansible_inventory_ini():
    """
    Function to find ansible-inventory.ini in the current directory or
    parent directory.
    """
    current_dir = os.getcwd()
...
def enable_disable_interfaces_menu():
    ...
    terminal_menu = TerminalMenu(
        ...
        menu_cursor_style=("fg_red", "bold"),
        menu_highlight_style=("bg_green", "bold"),
        title="Enable or Disable Interfaces",
    )
    menu_entry_index = terminal_menu.show()
    if menu_entry_index == len(options) - 1:
        interface_management_menu()
        ...
        # Return to the enable/disable interfaces menu after execution
        enable_disable_interfaces_menu()
...
def impair_interfaces_menu():
    ...
    for link in links:
        ...
        impairments_str = (
            ", ".join(impairments) if impairments else "No impairments"
        )
        options.append(
            (
                f"{link.source_host}:{link.source_interface} -> "
    ...
    if menu_entry_index == len(options) - 1:
        ...
        # Return to the impair interfaces menu after execution
        impair_interfaces_menu()
...
def manage_impairment(link):
    ...
    if menu_entry_index == len(options) - 1:
        ...
        # Apply the impairments after setting them
        apply_impairments(link)
        # Return to the manage impairment menu after setting an impairment
...
def apply_impairments(link):
    ...
    command = (
        f"sudo containerlab tools netem set -n {host.hostname} "
...
def enable_disable_interfaces(link):
    """
    Function to enable or disable both the source and destination network
        interfaces.
    This is being done because the abstraction of running a VM inside of a
        container prevents the interface state from being detected.
    If you want to manage the interfaces of a physical device, or another
        system that does not have this limitation you can use the
        following code:
    ```
    def enable_disable_interfaces(link):
    ...
    if not source_host:
        print(
            f"Source host {link.source_host} not found in the database. "
            f"Only managing destination interface."
        )
    elif not destination_host:
        print(
            f"Destination host {link.destination_host} not found in the "
            f"database. Only managing source interface."
        )
    else:
        print(
    ...
    try:
        if source_command:
            connect_to_host(
                source_host.hostname, source_command, return_to_main_menu=False
            )
        if destination_command:
            connect_to_host(
...
def single_host_backup_menu():
    ...
    terminal_menu = TerminalMenu(
        ...
        menu_cursor_style=("fg_red", "bold"),
        menu_highlight_style=("bg_green", "bold"),
        title="Backup Individual Host",
    )
    menu_entry_index = terminal_menu.show()
    # Logic to handle either backing up a host
    #   or returning to the backup configuration menu
    if menu_entry_index == len(options) - 1:
        config_backup_menu()
    else:
...
def backup_host_config(host):
    ...
    if host.network_os == "junos":
        command = (
            f"sshpass -p '{host.password}' scp -O -o StrictHostKeyChecking=no "
            f"-o UserKnownHostsFile=/dev/null "
            f"{host.username}@{host.ip_address}:/config/juniper.conf.gz "
            f"{backup_dir}/{host.hostname}_juniper.conf.gz"
        )
    else:
        print(f"Unsupported network OS: {host.network_os}")
        return
